...
function loadFixture(
  name: string,
): Array<{ ts: number; state: CursorState | null }> {
  const lines = readFileSync(resolve("fixtures/recordings", name), "utf-8")
    .trim()
    .split("\n");
...
describe("web: chat tab switching", () => {
  let env: ReturnType<typeof createTestEnv>;
  ...
  function tabSwitchState(): CursorState {
    return {
      connected: true,
      extractorStatus: "ok",
      ...
      messages: [
        { type: "human", id: "m1", flatIndex: 0, text: "Chat A message" },
        {
          type: "assistant",
          id: "m2",
          flatIndex: 1,
          text: "Reply from chat A",
          html: "<p>Reply from chat A</p>",
        },
      ],
      pendingApprovals: [],
      inputAvailable: true,
  ...
  it("replaces messages when the active chat tab changes", () => {
    fireFullState(env.mockSocket, tabSwitchState());
    assert.match(
      env.document.getElementById("messages")!.textContent!,
      /Chat A message/,
    );
    ...
    firePatch(env.mockSocket, {
      ...
      messages: [
        { type: "human", id: "m9", flatIndex: 0, text: "Chat B message" },
      ],
    });

    const text = env.document.getElementById("messages")!.textContent!;
    assert.match(text, /Chat B message/);
    assert.doesNotMatch(text, /Chat A message/);
  });
});
...
describe("web: message rendering", () => {
  ...
  it("renders assistant message", () => {
    ...
    const msgs = env.document.getElementById("messages")!;
    assert.ok(
      msgs.querySelector(".el-assistant"),
      "Should render assistant message (.el-assistant)",
    );
  });
  ...
  it("updates messages on patch", () => {
    ...
    assert.ok(
      ...
      `Expected more messages after patch, got ${initialCount} -> ${updatedCount}`,
    );
  });
});
...
describe("web: approval widgets", () => {
  ...
  it("renders run_command with Skip and Run buttons", () => {
    ...
    assert.ok(
      buttons.length >= 2,
      `Expected 2+ run buttons, got ${buttons.length}`,
    );
  });
  ...
  it("preserves command text across updates", () => {
    ...
    const hasNpmTest = Array.from(runCards).some((card) =>
      card.textContent!.includes("npm test"),
    );
...
describe("web: fetch tool", () => {
  ...
  it("renders fetch tool with approval buttons", () => {
    ...
    assert.ok(
      buttons.length >= 2,
      `Expected 2+ action buttons, got ${buttons.length}`,
    );
  });
...
describe("web: questionnaire widget", () => {
  ...
  it("hides questionnaire bar when questionnaire is null", () => {
    ...
    assert.ok(
      ...
      "Questionnaire bar should be hidden",
    );
  });
  ...
  it("shows questionnaire bar with questions", () => {
    ...
    assert.ok(
      !bar.classList.contains("hidden"),
      "Questionnaire bar should be visible",
    );
  ...
  it("disables continue button when continueDisabled is true", () => {
    ...
    fireFullState(env.mockSocket, state);
    const btn = env.document.getElementById(
      "btn-q-continue",
  ...
  it("hides questionnaire bar when questionnaire becomes null via patch", () => {
    ...
    assert.ok(
      bar.classList.contains("hidden"),
      "Should hide after patch with null",
    );
  });
  ...
  it("marks active question with active class", () => {
    ...
    assert.ok(
      !questions[0].classList.contains("questionnaire-question-active"),
    );
