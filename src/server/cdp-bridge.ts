...
import type { CursorWindow, ServerConfig } from "./types.js";
 ...
 */
export async function extractWorkspaceName(
  client: CdpClient,
  ...
  try {
    ...
    const { path, authority } = JSON.parse(raw) as {
      path: string;
      authority: string;
    };
    if (!path) return null;
...
function authorityToQualifier(authority: string): string {
  ...
  if (authority.startsWith("ssh-remote+")) {
    ...
    try {
      const decoded = JSON.parse(Buffer.from(hex, "hex").toString("utf8")) as {
        hostName?: string;
      };
...
export class CDPBridge extends EventEmitter {
  ...
  private resolvedEndpoint: ResolvedCdpEndpoint | null = null;
  ...
  async connect(targetId?: string): Promise<void> {
    try {
      ...
      console.log(
        `[cdp-bridge] Connecting to target: "${target.title}" (${target.url})`,
      );
      ...
      this._activeWorkspaceName = await extractWorkspaceName(
        this.client,
      ...
      if (this._activeWorkspaceName) {
        ...
        console.log(
          `[cdp-bridge] Workspace name: "${this._activeWorkspaceName}"`,
        );
      }
  ...
  private async getResolvedEndpoint(): Promise<ResolvedCdpEndpoint> {
    if (
      !this.resolvedEndpoint ||
      this.resolvedEndpoint.originalUrl !== this.config.cdpUrl
    ) {
      const endpoint = await resolveCdpEndpoint(this.config.cdpUrl);
      const configuredHost = new URL(this.config.cdpUrl).hostname;
      if (endpoint.host !== configuredHost) {
        console.log(
          `[cdp-bridge] Resolved CDP host ${configuredHost} → ${endpoint.host}`,
        );
      }
  ...
  private async fetchTargets(verbose = false): Promise<CDPTarget[]> {
    const endpoint = await this.getResolvedEndpoint();
    const url = `${endpoint.baseUrl}/json`;
    ...
    for (const target of targets) {
      target.webSocketDebuggerUrl = rewriteCdpWebSocketUrl(
    ...
    if (verbose) {
      ...
      const summary = Object.entries(
        rest.reduce<Record<string, number>>((acc, t) => {
        ...
        .join(", ");
      console.log(
        `[cdp-bridge] Found ${pages.length} page(s)${summary ? ` (+${summary})` : ""}:`,
      );
    ...
    return targets
      ...
      .map((t) => {
        ...
        if (t.id === this._activeTargetId && this._activeWorkspaceName) {
          return {
            id: t.id,
            title: this._activeWorkspaceName,
            url: t.url,
